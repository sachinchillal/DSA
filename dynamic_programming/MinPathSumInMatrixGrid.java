...
public class MinPathSumInMatrixGrid {
  ...
  static TestCaseArray[] TestCases = {
      ...
      new TestCaseArray(new int[][] { { 0, 0, 1 }, { 1, 0, 0 }, { 1, 0, 0 } }, 0),

      new TestCaseArray(new int[][] { { 1, 2, 3 }, { 4, 8, 2 }, { 1, 5, 3 } }, 11),
      new TestCaseArray(new int[][] { { 1, 2, 3 }, { 4, 2, 5 }, { 7, 8, 9 } }, 19),
  };
  ...
  static int maxPathSumInGrid(int[][] grid) {
    ...
    for (int i = 1; i < rows; i++) {
      for (int j = 1; j < cols; j++) {
        ...
        // dp[i][j] = Math.min(dp[i][j], diagonal + grid[i][j]);
      }
    }

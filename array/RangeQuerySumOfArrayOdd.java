...
public class RangeQuerySumOfArrayOdd {
  static TestCaseArray[] TestCases = {
      new TestCaseArray(new int[] { 1, 2, 3, 4 }, new int[][] { { 0, 2 }, { 1, 2 }, { 1, 4 } }, new int[] { 2, 2, 6 }),
  };
  ...
  public static int[] rangeQuerySumOfArrayEvenIndices(int[] A, int[][] Q) {
    ...
    // prefix sum Odd indices
    int[] pSumOdd = new int[n];
    pSumOdd[0] = 0; // No odd index at 0
    for (int i = 1; i < n; i++) {
      if (i % 2 == 0) { // only for even indices
        pSumOdd[i] = pSumOdd[i - 1]; // carry forward the previous sum
      } else {
